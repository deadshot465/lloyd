...
(defn/a get-eval-result [header token ctx]
    ...
    (setv response-body (dict (.loads json request.text)))
    (setv error-message "I can't evaluate your input because there's an error!")
    (setv error-occurred False)
    ...
    (if (= error-occurred True)
        (do (await (.send ctx error-message))
            (return True)))
